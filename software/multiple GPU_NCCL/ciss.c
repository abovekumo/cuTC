...
)
{
    ...
    #endif
    //create the entries
    idx_t bitmap, tilesize = (nnze - nnzb)/DEFAULT_T_TILE_LENGTH + 1;
...
)
{
    ...
    for(i = 0; i < gpunum; i++)
    {
        resultensor->cissunits[i] = cissbasic_alloc(newtensor, mode, resultensor->d_ref[i], resultensor->d_ref[i+1]);
