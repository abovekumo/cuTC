                                 ...
                                 idx_t tileend)
{
  ...
  idx_t tileid = bid * SGD_DEFAULT_BLOCKSIZE + tid + tilebegin;
  //idx_t tileid = bid * DEFAULT_BLOCKSIZE + tid;
  double * entries = d_traina->entries;
  idx_t localtile = tileid*((SGD_DEFAULT_T_TILE_LENGTH + 1) * DEFAULT_T_TILE_WIDTH);
  //buffer for matrices
  ...
  if(tileid < tileend)
  {
    ...
    if(bitmap != 0)
    {
      ...
      for(idx_t j = 0; j < SGD_DEFAULT_T_TILE_LENGTH/2; j++)
      {
            ...
            idx_t * max_badepochs)
{
    ...
    cissbasic_t * h_traina = cissbasic_alloc(traina, 0, traina->ind[0][0], (traina->ind[0][traina->nnz - 1] + 1), SGD_DEFAULT_T_TILE_LENGTH);
    #ifdef MCISS_DEBUG
  ...
  #pragma omp parallel
  {
    ...
    idx_t tilenum = nnz/SGD_DEFAULT_T_TILE_LENGTH + 1;
  ...
  #pragma omp parallel
  {
    //prepare the threads
    unsigned int cpu_thread_id = omp_get_thread_num();
    idx_t blocknum_m = (tileptr[cpu_thread_id + 1] - tileptr[cpu_thread_id] - 1)/SGD_DEFAULT_BLOCKSIZE + 1;
    ...
    #endif
    p_update_sgd_gpu<<<blocknum_m, SGD_DEFAULT_BLOCKSIZE, 0>>>(d_traina[cpu_thread_id], d_factora[cpu_thread_id], d_factorb[cpu_thread_id], d_factorc[cpu_thread_id], d_value_ha[cpu_thread_id], d_value_hb[cpu_thread_id], d_value_hc[cpu_thread_id], learning_rate, regularization_index, SGD_DEFAULT_BLOCKSIZE, SGD_DEFAULT_T_TILE_LENGTH, tileptr[cpu_thread_id], tileptr[cpu_thread_id + 1]);      
