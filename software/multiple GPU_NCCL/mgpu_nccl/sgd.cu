...
__global__ void p_pre_sgd_gpu(cissbasic_t * d_traina, 
                         ordi_matrix * d_factora, 
                         ordi_matrix * d_factorb, 
                         ordi_matrix * d_factorc)
{
  ...
  if(tileid < I)
  {
    for(idx_t i = 0; i < DEFAULT_NFACTORS; i++)
    {
      d_factora->values[tileid * DEFAULT_NFACTORS + i] = d_factora->values[tileid * DEFAULT_NFACTORS + i]/(double)DEFAULT_GPUNUM;    
    }
  }
  if(tileid < J)
  {
    for(idx_t i = 0; i < DEFAULT_NFACTORS; i++)
    {
      d_factorb->values[tileid * DEFAULT_NFACTORS + i] = d_factorb->values[tileid * DEFAULT_NFACTORS + i]/(double)DEFAULT_GPUNUM;    
    }
  }
  if(tileid < K)
  {
    for(idx_t i = 0; i < DEFAULT_NFACTORS; i++)
    {
      d_factorc->values[tileid * DEFAULT_NFACTORS + i] = d_factorc->values[tileid * DEFAULT_NFACTORS + i]/(double)DEFAULT_GPUNUM;    
    }
  }
            ...
            idx_t * max_badepochs)
{
  ...
  #pragma omp parallel
  {
    ...
    #ifdef DMMODIFY
    printf("my cpu thread id is %d, my device is %d\n", cpu_thread_id,cpu_thread_id % deviceCount);
    HANDLE_NCCLERR(ncclCommInitRank(&(comms[cpu_thread_id % deviceCount]), deviceCount, id, cpu_thread_id % deviceCount));
    #endif
  ...
  #pragma omp parallel
  {
    ...
    idx_t blocknum_p = maxdlength/ DEFAULT_BLOCKSIZE;
    p_pre_sgd_gpu<<<blocknum_p, DEFAULT_BLOCKSIZE, 0>>>(d_traina[cpu_thread_id], d_factora[cpu_thread_id], d_factorb[cpu_thread_id], d_factorc[cpu_thread_id]);
    #endif
    ...
    #pragma omp barrier  
    #ifdef DMMODIFY
    HANDLE_NCCLERR(ncclAllReduce((const void*)d_value_a[cpu_thread_id], (void*)d_value_a[cpu_thread_id], mats[0]->I * DEFAULT_NFACTORS, ncclDouble, ncclSum,  comms[cpu_thread_id], s));
    HANDLE_NCCLERR(ncclAllReduce((const void*)d_value_b[cpu_thread_id], (void*)d_value_b[cpu_thread_id], mats[1]->I * DEFAULT_NFACTORS, ncclDouble, ncclSum,  comms[cpu_thread_id], s));
    HANDLE_NCCLERR(ncclAllReduce((const void*)d_value_c[cpu_thread_id], (void*)d_value_c[cpu_thread_id], mats[2]->I * DEFAULT_NFACTORS, ncclDouble, ncclSum,  comms[cpu_thread_id], s));
